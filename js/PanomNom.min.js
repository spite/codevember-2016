	...
	this.service.getPanoramaById( id, function( result, status) {
		...
		for( var y = 0; y < h; y++ ) {
			for( var x = 0; x < w; x++ ) {
				...
				var url = 'https://lh3.ggpht.com/' + id + '/x' + x +'-y' + y + '-z' + zoom + '/p';
				console.log( url );
...
PANOMNOM.GooglePhotoSphereLoader.prototype.loadFromService = function( result, zoom ) {

	//console.log( 'Loading ' + id + ' ' + zoom );

	this.zoom = zoom;
	this.panoId = result.location.pano.replace( 'F:', '' );
	...
	for( var y = 0; y < h; y++ ) {
		for( var x = 0; x < w; x++ ) {
			...
			var url = 'https://lh3.ggpht.com/' + this.panoId + '/x' + x +'-y' + y + '-z' + zoom + '/p';
